...
def move_downloaded_files_under_sorted_folder():
    ...
    while not stop_event.is_set(): # While the program is still running
        websiteName = findWebsite(pathToPytesseract)
...
def main():
    global stop_event, pathToPytesseract, folderPath
    ...
    try:
        ...
        # Main tab contents
        cur_path_to_tesseract = tk.Label(mainTab, text=(f"Pytesseract Path: {pathToPytesseract}"))
        cur_path_to_tesseract.pack(pady=5)
        tk.Button(mainTab, text="Change Path to Pytesseract\'s tesseract.exe", command=lambda:
            change_path_to_tesseract(pathToPytesseract, store_pytesseract_file_path,updateLog,cur_path_to_tesseract)).pack(pady=5)

        # Access the stored pytesseract file path and prompt the user to input a valid one if none exists
        pathToPytesseract = verify_path_to_tesseract(pathToPytesseract, get_stored_pytesseract_path, store_pytesseract_file_path, updateLog, cur_path_to_tesseract)
        ...
        tk.Button(mainTab, text="Change Operating Folder", command=lambda:
            change_path_to_sort_folder(folderPath,store_sort_folder_file_path,updateLog,cur_path_to_tesseract)).pack(pady=5)
