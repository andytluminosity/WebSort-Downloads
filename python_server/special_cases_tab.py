...
def delete_special_case(row, labels, dbTab, logText):
    ...
    c.execute(
        "SELECT websiteName FROM websites ORDER BY websiteName ASC LIMIT 1 OFFSET ?",
        (row - 1,),
    )
    websiteName = c.fetchone()[0]
    if tk.messagebox.askyesno(
        "Confirm Deletion", f"Are you sure you want to delete '{websiteName}'?"
    ):
...
def open_add_special_case_window(logText, labels, dbTab):
    ...
    def add_website(logText):
        ...
        if folderPath:
            ...
            if not check_existence_of_websiteName(
                websiteName
            ):  # Store the special case if one doesn't already exist for the website
                store_special_case(websiteName, folderPath)
                updateLog(
                    f'Added Special Case: All files from "{websiteName}" will be moved to {folderPath}',
                    logText,
                )
                ...
                if tk.messagebox.askyesno(
                    ...
                    f"A special case already exists "
            ...
            tk.messagebox.showinfo("Error", "Please select a folder path")
    ...
    def select_folder_path():
        ...
        folderPath = askdirectory(
            title="Select the folder to move downloads from the indicated website to",
            mustexist=True,
        )
    ...
    folder_path_entry = tk.Button(
        add_window, text="Select Folder Path", command=select_folder_path
    )
    folder_path_entry.grid(row=2, column=1, columnspan=2, padx=10, pady=10, sticky="ew")

    tk.Button(add_window, text="Add", command=lambda: add_website(logText)).grid(
...
def refresh_special_cases_gui(labels, special_cases_tab, logText):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT websiteName, folderPath FROM websites ORDER BY websiteName ASC"
    )
    ...
    for row, (websiteName, folderPath) in enumerate(logs):
        new_website_label = tk.Label(
            special_cases_tab, text=f"{websiteName}", anchor="w"
        )
        ...
        delete_button = tk.Button(
            special_cases_tab,
            text="X",
            command=lambda row=row + 1: delete_special_case(
                row, labels, special_cases_tab, logText
            ),
        )
