...
def getMostRecentFileInFolder(folderPath):
    ...
    )  # Refresh the list of files seen by the program
...
def verify_path_to_sort_folder(
    ...
    if stored_path:
        ...
        cur_sort_folder.config(
            text=f"Operating Folder: {folderPath}"
        )  # Update the displayed path on GUI
        return folderPath
    else:
        while True:  # While a path has not been entered
            # Prompt the user to input the file path
            input_path = askdirectory(
                title="Select the Folder to Move Downloads From and Store Sorted Folders In",
                mustexist=True,
            )
            ...
            if input_path:
                ...
                messagebox.showinfo(
                    "Error",
                    "Please select the valid folder path to move downloads from "
                    "and store sorted folder in",
                )


def change_path_to_sort_folder(
    ...
    if input_path:
        ...
        messagebox.showinfo(
            "Error",
            "Please select a valid folder path to move downloads from and store sorted folder in",
        )
